...
INSERT INTO ETL_CONTROL (step_id, step_name, step_sequence, source_query, target_table, is_active) 
VALUES (1, 'Extract Employees', 1, 
  'INSERT INTO ETL_EMPLOYEES_STAGING 
   ...
   WHERE e.salary > 0 AND e.employee_id IN 
         (SELECT employee_id FROM SAMPLE_EMPLOYEES WHERE department IS NOT NULL)',
  'ETL_EMPLOYEES_STAGING', 1);

INSERT INTO ETL_CONTROL (step_id, step_name, step_sequence, source_query, target_table, is_active) 
VALUES (2, 'Calculate Bonus', 2, 
  'UPDATE ETL_EMPLOYEES_STAGING ees 
   SET ees.bonus = CASE 
...
CREATE OR REPLACE PROCEDURE execute_etl_from_control_table
AS
    CURSOR etl_steps IS
...
BEGIN
    ...
    FOR etl_rec IN etl_steps LOOP
        ...
        -- Log the execution
        l_log_id := etl_log_seq.NEXTVAL;
        INSERT INTO ETL_LOG (log_id, step_id, step_name, execution_start_time, execution_end_time, 
...
BEGIN
    ...
    execute_etl_from_control_table();
END;
/

-- Step 8: CHECK RESULTS
SELECT * FROM ETL_LOG ORDER BY log_id;
SELECT * FROM ETL_EMPLOYEES_STAGING;
